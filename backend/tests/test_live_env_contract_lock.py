...
def test_s1b_forbidden_set_is_exactly_closeout_and_disjoint_from_allowed():
    assert MIGRATION_LIVE_FORBIDDEN == {CLOSEOUT_REV}, (
        f"C must be exactly {{{CLOSEOUT_REV}}}; got {sorted(MIGRATION_LIVE_FORBIDDEN)}"
    )
...
def test_s2_sets_are_complete_and_disjoint():
    ...
    assert len(chain) == 12 and len(MIGRATION_LIVE_APPLICABLE) == 11 and len(MIGRATION_LIVE_FORBIDDEN) == 1, (
        f"12 == 11 + 1 expected; got chain={len(chain)}, B={len(MIGRATION_LIVE_APPLICABLE)}, "
