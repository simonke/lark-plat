...
### 三期
工单、知识库、CI/CD 集成、CMDB 深化、编排（Playbook）

> 2026-09-24 立项补全（刘辉裁定）：「三期」单义＝ **5 项**；新增 **CMDB 深化**（一期 `asset`/`asset_host` 为浅台账，深化＝关系/拓扑/影响分析）、**编排 Playbook** 单列；**CI/CD 集成**收敛为「发布编排段」（不自造 CI 引擎）。
