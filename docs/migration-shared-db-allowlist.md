...
head = `e9d8c7b6a5f4`（唯一；P4 AIOps `ops_event`/`kb_embedding`/`ai_action`/`ai_eval_case`/`ai_eval_run`）。
...
| **C** | `MIGRATION_LIVE_FORBIDDEN` | **禁落**共享库的迁移 | `{e8a1b2c3d4f5, c9e3f1a2b4d6, e1f2a3b4c5d7, f2a3b4c5d6e7, a1b2c3d4e5f7, b2c3d4e5f6a8, e9d8c7b6a5f4}` |
...
1. **完备性**：`链 ⊆ B ∪ C` 且 `B ∩ C = ∅`（18 = 11 + 7，**零空洞**）。
2. **可停留性**：`A ⊆ B`，且 A 在链上**连续**（`d4e5f6a7b8c9 → c3d4e5f6a7b8`），为 B 内「当前允许停留」的显式子集。
3. **已分类**：`head ∈ B ∪ C`（本批 head `e9d8c7b6a5f4` ∈ C）。

> ⚠️ 完备性**必须**用 `B ∪ C`。**严禁**用 `A ∪ C`（仅 3/18 覆盖 ⇒ 必误红）。
